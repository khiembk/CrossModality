...
def load_ecg(root, batch_size, valid_split=-1):
    ...
    with open(root + '/challenge2017.pkl', 'rb') as fin:
        res = pickle.load(fin)
    ## scale data
    print("Keys in the pickle file:", res.keys())
    all_data = res['data']
...
class NSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(self.file_path, "r") as f:
            ...
            print("index sample shape: ", idx_cfd)
            ...
            # Crop to 224x224 for SwinBase compatibility (if necessary)
            if data.shape[0] > 224:
                data = data[:224, :224, :, :]
            ...
            x = data[..., :initial_step, :].permute(3, 0, 1, 2)  # Shape: (channels=6, x, y, time)
...
class MatReader(object):
    ...
    #         self.old_mat = True
    #     except:
    #         print("wrong file path: ", self.file_path)
    #         self.data = h5py.File(self.file_path)
    #         self.old_mat = False
    
    def _load_file(self):
        try:
            # Try reading as a legacy MAT file (version < 7.3)
            self.data = scipy.io.loadmat(self.file_path)
            self.old_mat = True
        except ValueError as e:
            print(f"Failed to load using scipy.io.loadmat: {e}")
            try:
                # Fallback to h5py for version >= 7.3 MAT files (HDF5 format)
                self.data = h5py.File(self.file_path, 'r')
                self.old_mat = False
            except OSError as e:
                print(f"Failed to load using h5py: {e}")
                raise Exception(f"Unable to load the file: {self.file_path}")
...
class UnitGaussianNormalizer(object):
    ...
    # def decode(self, x, sample_idx=None):
    # # Determine the appropriate std and mean based on sample_idx
    #   if sample_idx is None:
    #     std = self.std + self.eps # n
    #     mean = self.mean
    #   else:
    ...
    # # Ensure std and mean are broadcastable with x
    #   if std.shape != x.shape:
    #     std = std.expand_as(x)  # Adjust std to match x dimensions
    #   if mean.shape != x.shape:
    #     mean = mean.expand_as(x)  # Adjust mean to match x dimensions
